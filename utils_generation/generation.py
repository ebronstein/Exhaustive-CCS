    ...
    with torch.no_grad():
        ...
        for key, record in zip(frame_dict.keys(), records):
            ...
            if args.cal_zeroshot or args.cal_logits:
                ...
                for idx in range(len(frame)):
                    if is_decoder_only or is_encoder_only:
                        ...
                        logits = [
                            model(input_ids, labels=w)["logits"]
                            for w in ans_token
                        ]
                ...
                if args.cal_zeroshot:
                    ...
                    if args.print_more:
                        print(
                            "Finish calculating the zero-shot accuracy for {} data in {}.".format(
                                len(frame), key
                            )
                        )
            ...
            if args.cal_hiddenstates:
                ...
                if (
                    args.states_location == "decoder" and is_encoder_only
            ...
            print(
                "{} s/data. {}".format(
                    round((time.time() - start) / record["population"], 2),
                    record,
                )
            )
...
def getLogProbs(logits, ans_token, mdl_name):
    ...
    if "gpt" in mdl_name or "llama" in mdl_name:
        return [
            torch.mean(
                logit.log_softmax(dim=-1)[range(-ans.shape[1] - 1, -1), ans[0]]
            )
        ...
        return [
            torch.mean(
                logit.log_softmax(dim=-1)[range(ans.shape[1] - 1), ans[0][:-1]]
            )
            for logit, ans in zip(logits, ans_token)
        ]
